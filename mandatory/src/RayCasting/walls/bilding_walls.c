...
/*   Updated: 2023/11/15 15:06:50 by hamaarou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	render3dwalls(t_mlx *mlx, int nb)
{
	...
	mlx->tex_y = 0;
	mlx->tex_x = get_x(mlx);
	ray_distance = mlx->distance * cos(mlx->rayangle
			- mlx->cub3d.player.rotation_angle);
	distance_pjplane = (MAP_W / 2) / tan(FOV_ANGLE / 2);
	...
	while (j < wallbop)
	{
		...
		mlx->tex_y = distance_fromtop * (mlx->no_texture->height
				/ wall_strip_height);
		color = 0;
		getdirection(mlx);
		if (mlx->x_offset == 0)
		{
			mlx->tex_y = distance_fromtop * (mlx->so_texture->height
					/ wall_strip_height);
		...
		{
			mlx->tex_y = distance_fromtop * (mlx->ea_texture->height
					/ wall_strip_height);
			color = get_color(mlx->ea_texture, mlx->tex_x, mlx->tex_y);
		}
		else if (mlx->x_offset == 3)
		{
			mlx->tex_y = distance_fromtop * (mlx->we_texture->height
					/ wall_strip_height);
